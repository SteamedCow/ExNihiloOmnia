...
public class InfestedLeavesRenderer extends TileEntitySpecialRenderer<TileEntityInfestedLeaves> {
    ...
    @Override
    public void renderTileEntityFast(TileEntityInfestedLeaves te, double x, double y, double z, float partialTicks, int destroyStage, float partial, BufferBuilder buffer) {
        ...
        if (te.getState() != null) {
            ...
            for (BakedQuad quad : models.get(state)) {
                if (state.shouldSideBeRendered(world, pos, quad.getFace())) {
                    ...
                    bufferBuilder.putColorMultiplier(color.r, color.g, color.b, 1);
